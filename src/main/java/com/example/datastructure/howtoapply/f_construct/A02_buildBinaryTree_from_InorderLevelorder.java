package com.example.datastructure.howtoapply.f_construct;
...
public class A02_buildBinaryTree_from_InorderLevelorder {
	private static A02_buildBinaryTree_from_InorderLevelorder obj = new A02_buildBinaryTree_from_InorderLevelorder();
	
	
	public static void main(String[] args) {
		int inArr[] = {4, 8, 10, 12, 14, 20, 22};
		int levelArr[] = {20, 8, 22, 4, 12, 10, 14};
		Node root = obj.buildTree(inArr, levelArr);
		System.out.println(root);
	}
	
	
	private Node buildTree(int inArr[], int levelArr[]) {
		Node startnode = null;
		int inStart = 0;
	...
	private Node constructTree(Node startNode, int[] levelArr, int[] inArr, int inStart, int inEnd) {
		if(inStart > inEnd)
			return null;
		if(inStart == inEnd)
